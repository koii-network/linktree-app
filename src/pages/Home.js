...
import { allLinktrees, getAuthList, getLinktree, transferKoii } from "../api";
...
const HomePage = () => {
    ...
    if (isFinnieDetected) {
      ...
      try {
        if (pubKey) {
          ...
          const isAuthenticated = await getAuthList(pubKey, nodeList);
  ...
  const handleTransferKoii = async () => {
    try {
      const isTransfer = await transferKoii(nodeList);
